 ...
 * 섬 전체를 덮는 통행 격자.
 *
 * ⚠ 평균 반지름이 ISLAND_BASE_RADIUS(56)여도 하모닉이 최대 +1% 라 해안선은
 *   57m 까지 나가고, 헤엄이 거기서 4m 더 나간다. 격자가 그보다 좁으면 섬
 *   가장자리가 통째로 "갈 수 없는 곳"이 되고, 거기 놓인 오브젝트는 영영
 *   도달할 수 없어진다. 여유를 두고 ±68 로 잡는다.
 *
 * 0.5m 칸이면 캐릭터(반지름 0.35m)가 나무 사이를 지나갈 수 있는지 판정하기에
 * 충분하다. 칸이 7만 개라 마스크는 74KB.
 */
