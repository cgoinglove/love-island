...
function shellSpeedFor(power: number): number {
   ...
   * 규모가 클수록 조금 더 높이 오른다.
   *
   * 취향이 아니라 **거리 보정**이다. 밤의 큰 발은 깃발보다 먼 바다(64~78m)에서
   * 터지는데, 곡률이 거리 제곱으로 끌어내리므로(110m 에서 16m) 같은 높이로 쏘면
   * 수평선 아래로 가라앉는다. 규모와 거리가 같이 가는 구조라 배수 하나로 맞는다.
   */
  return SHELL_SPEED + (power - 1) * 2.6;
}
