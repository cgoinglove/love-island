 ...
 * 찌가 떨어지는 거리(m).
 *
 * 9m 였다가 6.5m 로 당겼다가 8m 로 돌아왔다. 화면에서 찌가 왼쪽 가장자리에
 * 붙지 않을 만큼은 가까워야 하고(카메라 화각이 24m 거리에서 좌우 ±12m),
 * 찌가 뜬 자리의 물이 모래처럼 보이지 않을 만큼은 깊어야 한다.
 * 섬이 세 배가 되면서 남쪽 모래톱이 길고 완만해져 그 균형점이 8m 로 옮겨갔다.
 */
export const CAST_DISTANCE = 8;
